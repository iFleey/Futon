...
std::memcpy(host_mem_addr, kTestInput0Tensor,
...
std::memcpy(host_mem_addr, kTestInput1Tensor,
...
"New size: %zu", resized_input_tensor_size);
EXPECT_EQ(resized_input_tensor_size,
1 * 2 * 3 * sizeof(float));
...
ASSERT_EQ(layout
.rank, sizeof(expected_output_tensor_shapes) / sizeof(int));
...
++i) {
EXPECT_EQ(layout
.dimensions[i], expected_output_tensor_shapes[i]);
}
