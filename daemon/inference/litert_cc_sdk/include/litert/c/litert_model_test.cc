...
type.
first = kId;
type.second.unranked_tensor_type.
element_type = kElementType;
...
LiteRtTensorT tensor;
tensor.
SetType(MakeRankedTensorType(kElementType, {3, 3})
);
...
ASSERT_EQ(actual_user_arg_index,
0);
...
ASSERT_EQ(per_channel_quantization
...
op.
SetOpCode(kLiteRtOpCodeTflCustom);
const char *code;
...
EXPECT_EQ(BufferRef(metadata, metadata_size)
.
