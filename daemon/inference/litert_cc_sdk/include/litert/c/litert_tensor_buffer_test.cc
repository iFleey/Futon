...
.layout.rank, 1);
ASSERT_EQ(tensor_type
.layout.dimensions[0], kTensorType.layout.dimensions[0]);
ASSERT_EQ(tensor_type
...
std::memcpy(host_mem_addr, kTensorData,
sizeof(kTensorData));
...
ASSERT_EQ(std::memcmp(host_mem_addr, kTensorData, sizeof(kTensorData)),
0);
ASSERT_EQ(LiteRtUnlockTensorBuffer(tensor_buffer), kLiteRtStatusOk
);
...
<< "OpenCL could not be loaded; skipping the test";
}
