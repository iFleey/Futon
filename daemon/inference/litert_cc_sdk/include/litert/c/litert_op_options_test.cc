...
ASSERT_EQ(asymmetric_quantize_input,
false);
}
...
ASSERT_EQ(new_shape_size,
-1);  // Remains unchanged in case of error.
}
...
EXPECT_EQ(reshape_op
.

OpCode(), kLiteRtOpCodeTflReshape
...
ASSERT_EQ(new_shape_size,
2);
ASSERT_EQ(new_shape[0],
3);
ASSERT_EQ(new_shape[1],
2);
}
...
LITERT_ASSERT_OK_AND_ASSIGN(litert::Subgraph
subgraph, model.
...
ASSERT_STREQ(parsed_attributes["an_attribute"]
.
...
"foo");
ASSERT_EQ(parsed_attributes["meaning_of_life"]
.
...
ASSERT_EQ(num_squeeze_dims,
1);
ASSERT_EQ(squeeze_dims[0],
1);
}
