...
EXPECT_EQ(ErrorWithCustomStatus(),
...
EXPECT_DEATH(
        LITERT_ABORT_IF_ERROR(InvalidArgumentError)
<< "Error abort log",
#ifndef NDEBUG
...
LITERT_ASSIGN_OR_ABORT(int v, Expected<int>(
...
Message(), HasSubstr(error_message_builder.str())
...
Message(), HasSubstr("An error message.")
...
intercepted_logs <<
log_interceptor;
EXPECT_THAT(intercepted_logs
.

str(), HasSubstr("Failed a subcall.")

);
EXPECT_THAT(intercepted_logs
.

str(), HasSubstr("An error message.")
...
auto error_2 = []() -> absl::StatusOr<int> {
    LITERT_ASSIGN_OR_RETURN(
            auto
    v, Expected < int > (Unexpected(kLiteRtStatusErrorInvalidArgument,
                                    "An error message.")));
    return v;
}();
EXPECT_FALSE(error_2
.
