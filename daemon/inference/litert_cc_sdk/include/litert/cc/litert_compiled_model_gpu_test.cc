...
namespace litert {
    namespace {
        ...
        Expected <Options> CreateGpuOptions(bool external_tensors_mode) {
            LITERT_ASSIGN_OR_RETURN(litert::Options
        ...
        void BasicTest(bool external_tensors_mode) {
            ...
            LITERT_ASSERT_OK_AND_ASSIGN(auto
            options,
...
LITERT_ASSERT_OK_AND_ASSIGN(
        auto options,
        CreateGpuOptions(/*no_immutable_external_tensors_mode=*/

true));

LITERT_ASSIGN_OR_ABORT(auto &runtime_options, options
...
LITERT_ASSERT_OK_AND_ASSIGN(auto num_events, profiler
...
ASSERT_EQ(events[0]
.event_source, ProfiledEventSource::LITERT);
...
.
GetOption(kLiteRtEnvOptionTagOpenClDeviceId)
);
ABSL_LOG(INFO)
<< "OpenCL device id: "
<< reinterpret_cast
...
.
GetOption(kLiteRtEnvOptionTagOpenClPlatformId)
);
ABSL_LOG(INFO)
<< "OpenCL platform id: "
<< reinterpret_cast
...
.
GetOption(kLiteRtEnvOptionTagOpenClContext)
);
ABSL_LOG(INFO)
<< "OpenCL context: "
<< reinterpret_cast
...
.
GetOption(kLiteRtEnvOptionTagOpenClCommandQueue)
);
ABSL_LOG(INFO)
...
compiled_model.
RunAsync(input_buffers, output_buffers, async_execution_mode
);
...
compilation_options->
SetHardwareAccelerators(accelerator_flags);
...
EnableExternalTensorsMode (CompiledModelGpuTest::GetParam())
...
ASSERT_TRUE(
        input_buffers[2]
.
Write<int64_t>(absl::MakeConstSpan(arg2_data, 1)
));
...
++i) {
ABSL_LOG(INFO)
<< "Result: " << output[i] << "\t" << expected_output[i];
}
...
auto compilation_options = Options::Create();
compilation_options->
SetHardwareAccelerators(HwAccelerators::kGpu);
LITERT_ASSERT_OK_AND_ASSIGN(auto &gpu_options,
                            compilation_options->GetGpuOptions());
LITERT_ASSERT_OK(
        gpu_options
...
auto compiled_model_res = CompiledModel::Create(
        ...
        *compilation_options);
EXPECT_FALSE(compiled_model_res
.

HasValue()

);
EXPECT_EQ(compiled_model_res
.
...
Status(), kLiteRtStatusErrorCompilation
...
Expected <std::vector<TensorBuffer>> CreateGlInputBuffers(
    ...
    for (auto &input_name: input_names) {
        ...
        LITERT_ASSIGN_OR_RETURN(size_t
        buffer_size,
...
Expected <std::vector<TensorBuffer>> CreateGlOutputBuffers(
    ...
    for (auto &output_name: output_names) {
        LITERT_ASSIGN_OR_RETURN(TensorBufferRequirements
        input_buffer_requirements,
...
<< "GPU tests are not supported in this configuration";
}
...
LITERT_ASSERT_OK_AND_ASSIGN(litert::Options
options,
...
LITERT_ASSERT_OK_AND_ASSIGN(auto &gpu_options, options
...
LITERT_ASSERT_OK(
        gpu_options
.
SetPrecision(GpuOptions::Precision::kFp32)
);
LITERT_ASSERT_OK(
        gpu_options
.
SetBufferStorageType(GpuOptions::BufferStorageType::kBuffer)
);
LITERT_ASSERT_OK(
        gpu_options
.
...
bool async_execution_mode = true;
compiled_model.
RunAsync(signature_index, input_buffers, output_buffers,
        async_execution_mode
);

ASSERT_TRUE(output_buffers[0]
.
...
LITERT_ASSERT_OK_AND_ASSIGN(auto output_event, output_buffers[0]
...
LITERT_ASSERT_OK_AND_ASSIGN(TensorBufferRequirements
requirements,
