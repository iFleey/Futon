...
namespace litert {
    namespace {
        ...
        ->second, kTensorData, sizeof(kTensorData));
    }
{
    auto lock_and_addr = TensorBufferScopedLock::Create(
...
<< "OpenCL buffers are not supported on this platform; "
"skipping the test";
}
if (!
...
<< "OpenCL library could not be loaded; skipping the test";
}
...
sizeof(kTensorData)));
LITERT_EXPECT_OK(tensor_buffer
.

Lock()

);
EXPECT_THAT(tensor_buffer
.
...
<< "ION buffers are not supported on this platform; "
"skipping the test";
}
...
LITERT_ASSERT_OK_AND_ASSIGN(auto tensor_type, tensor_buffer
...
LITERT_ASSERT_OK_AND_ASSIGN(auto size, tensor_buffer
...
LITERT_ASSERT_OK_AND_ASSIGN(auto offset, tensor_buffer
...
ASSERT_EQ(tensor_buffer
.

Get(), litert_tensor_buffer
...
std::memcpy(host_memory_ptr, kTensorData,
sizeof(kTensorData));
...
ASSERT_EQ(std::memcmp(read_data, kTensorData, sizeof(kTensorData)),
0);
}
...
<< "OpenCL and/or GL are not supported on this platform; "
"skipping the test";
}
// User provides CL-GL environment.
auto user_gpu_env = UserGpuEnvironment::Create();
ASSERT_TRUE(user_gpu_env
!= nullptr);
ASSERT_TRUE(user_gpu_env
->
...
<< "CL/GL sharing is not supported on this platform; "
"skipping the test";
}
...
GetEnvironment(), TensorBufferType::kGlBuffer,
        RankedTensorType(kTestTensorType),
...
<< "GL is not supported on this platform; skipping the test";
}
...
sizeof(kTensorData)));
LITERT_ASSERT_OK_AND_ASSIGN(TensorBuffer::GlBuffer
gl_buffer,
tensor_buffer.
...
#endif  // LITERT_HAS_OPENGL_SUPPORT
EXPECT_THAT(gl_buffer
.id, Ne(0));
EXPECT_THAT(gl_buffer
.size_bytes, Eq(sizeof(kTensorData)));
EXPECT_THAT(gl_buffer
.offset, Eq(0));
}
...
LITERT_ASSERT_OK_AND_ASSIGN(
        TensorBuffer
tensor_buffer_from_gl_buffer,
TensorBuffer::CreateFromGlBuffer(user_gpu_env
->
...
LITERT_ASSERT_OK_AND_ASSIGN(
        TensorBuffer::GlBuffer
gl_buffer_from_tensor_buffer,
tensor_buffer.

GetGlBuffer()

);
EXPECT_THAT(gl_buffer_from_tensor_buffer
.target,
Eq(gl_buffer
.target));
EXPECT_THAT(gl_buffer_from_tensor_buffer
...
<< "OpenGl and/or AHWB are not supported on this platform; "
"skipping the test";
}
// User provides EGL environment.
auto user_gpu_env = UserGpuEnvironment::Create();
...
<< "AHWB/GL interop is not supported on this platform; "
"skipping the test";
}
...
GetEnvironment(), TensorBufferType::kAhwb,
        RankedTensorType(kTestTensorType),

sizeof(kTensorData)));

// Write to AHWB Tensor buffer.
LITERT_ASSERT_OK(ahwb_tensor_buffer
.
Write<float>(absl::MakeConstSpan(
        kTensorData, sizeof(kTensorData) / sizeof(kTensorData[0]))
));
...
gl_buffer.size_bytes, gl_buffer.offset));
float read_data[sizeof(kTensorData) / sizeof(kTensorData[0])];
LITERT_ASSERT_OK(gl_buffer_from_ahwb
.
Read<float>(absl::MakeSpan(read_data)
));
ASSERT_EQ(std::memcmp(read_data, kTensorData, sizeof(kTensorData)),
0);
}
...
GetEnvironment(), TensorBufferType::kOpenClBuffer,
        RankedTensorType(kTestTensorType),

sizeof(kTensorData)));

LITERT_ASSERT_OK_AND_ASSIGN(auto cl_buffer, tensor_buffer
...
<< "OpenCL and/or AHWB are not supported on this platform; "
"skipping the "
"test";
}
auto user_gpu_env = UserGpuEnvironment::Create(/*create_gl_env=*/false);
...
<< "AHWB/CL interop is not supported on this platform; "
"skipping the test";
}
// Create AHWB Tensor buffer.
LITERT_ASSERT_OK_AND_ASSIGN(
...
LITERT_ASSERT_OK_AND_ASSIGN(cl_mem
cl_buffer,
ahwb_tensor_buffer.
...
LITERT_ASSERT_OK_AND_ASSIGN(
        TensorBuffer
cl_buffer_from_ahwb,
TensorBuffer::CreateFromClBuffer(user_gpu_env
->
...
std::memcpy(tensor_data, lock_and_addr
->second, sizeof(tensor_data));
...
LITERT_ASSERT_OK_AND_ASSIGN(auto memory, tensor_buffer
...
TensorBuffer::CreateManaged(env, TensorBufferType::kAhwb,
        RankedTensorType(kTestTensorType),
sizeof(kTensorData)));
LITERT_ASSERT_OK_AND_ASSIGN(AHardwareBuffer
* ahwb, tensor_buffer.
...
LITERT_ASSERT_OK_AND_ASSIGN(
        Event
event,
Event::CreateFromSyncFenceFd(env
.

Get(), kFakeSyncFenceFd,
...
EXPECT_TRUE(tensor_buffer
.

HasEvent()

);
LITERT_ASSERT_OK_AND_ASSIGN(Event
tensor_buffer_event,
tensor_buffer.
...
// Clear event.
LITERT_ASSERT_OK(tensor_buffer
.

ClearEvent()

);
EXPECT_FALSE(tensor_buffer
.
