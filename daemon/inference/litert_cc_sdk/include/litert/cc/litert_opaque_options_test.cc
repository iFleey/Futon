...
namespace litert {
    namespace {
        ...
        LITERT_ASSERT_OK_AND_ASSIGN(SimpleOptions
        opts,
        ...
        LITERT_ASSERT_OK_AND_ASSIGN(SimpleOptions::Payload
        * data,
...
LITERT_ASSERT_OK(options
.

Append (std::move(simple_options))
...
LITERT_ASSERT_OK(opts
.
SetHash(std_hash)
);

LITERT_ASSERT_OK_AND_ASSIGN(uint64_t
hash, opts.

Hash()

);
EXPECT_EQ(hash, std::hash<int>{}
(kPayloadValue));
}
