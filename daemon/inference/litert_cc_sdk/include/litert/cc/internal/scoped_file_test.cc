...
#define close _close
        ...
        ASSERT_FALSE(ScopedFile::Open(path.string())
        .
...
);  // NOLINT: use after move is intended to check
// the state.
}
...
ScopedFile uninitialized_file;
EXPECT_FALSE(uninitialized_file
.
...
EXPECT_EQ(file_contents, std::string(reference_data)
+ reference_data);
}
