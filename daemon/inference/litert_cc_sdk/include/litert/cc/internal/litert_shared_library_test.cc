...
namespace litert {
    namespace {
        ...
        EXPECT_THAT(static_cast

        <int>(RtldFlags::Now()), Eq(RTLD_NOW)

        );
        EXPECT_THAT(static_cast

        <int>(RtldFlags::Lazy()), Eq(RTLD_LAZY)
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(SharedLibrary
    lib,
    ...
    << "TestFunction symbol was stripped from binary.";
}

decltype(&TestFunction) test_function = maybe_test_function.Value();
ASSERT_NE(test_function,
nullptr);
...
LITERT_ASSERT_OK_AND_ASSIGN(
        SharedLibrary
lib,
SharedLibrary::Load(lib_path, RtldFlags::Now()
.
...
LITERT_ASSERT_OK_AND_ASSIGN(TestFunctionSignature
test_function,
