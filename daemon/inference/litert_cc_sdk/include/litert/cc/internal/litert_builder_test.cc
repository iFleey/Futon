...
OpInputs inputs;
inputs.
push_back(Tensor(&litert_tensor_0)
);
inputs.
push_back(Tensor(&litert_tensor_1)
);
OpOutputs outputs;
outputs.
push_back(Tensor(&litert_tensor_2)
);
...
cc_builder.
SetOpOptions<AddOptions>(op, std::move(add_options)
);
}
