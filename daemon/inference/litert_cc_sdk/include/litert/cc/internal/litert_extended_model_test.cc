...
//===----------------------------------------------------------------------===//
    ...
    Code(), kLiteRtOpCodeTflCustom

    );

    LITERT_ASSERT_OK_AND_ASSIGN(auto custom_code, op
...
ASSERT_EQ(input_ranked_tensor_type
->
...
EXPECT_TRUE(output_defining_op
.
...
Value(), ::testing::ElementsAreArray({1.0, 2.0, 3.0, 4.0})
...
.
first = kLiteRtQuantizationNone;

Tensor tensor(&litert_tensor);
EXPECT_EQ(tensor
.

QTypeId(), kLiteRtQuantizationNone
...
LiteRtTensorT litert_tensor;
litert_tensor.
SetQarams(MakePerTensorQuantization(kScale, kZeroPoint)
);
...
QTypeId(), kLiteRtQuantizationPerTensor
...
EXPECT_EQ(per_tensor_quantization
.scale, kScale);
EXPECT_EQ(per_tensor_quantization
.zero_point, kZeroPoint);
}
...
litert_tensor.
SetQarams(per_channel);

Tensor tensor(&litert_tensor);
ASSERT_EQ(tensor
.

QTypeId(), kLiteRtQuantizationPerChannel
...
EXPECT_EQ(per_channel_quantization
.num_channels, kNumChannels);
EXPECT_EQ(per_channel_quantization
.quantized_dimension, kQuantizedDimension);
}
...
ASSERT_EQ(output_ranked_tensor_type
->
...
LITERT_ASSERT_OK_AND_ASSIGN(auto metadata, model
