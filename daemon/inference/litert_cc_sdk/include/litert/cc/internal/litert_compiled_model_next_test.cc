...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation(0, "priority", "high"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value, compiled_model

.GetDispatchAnnotation(0, "priority"));
ASSERT_TRUE(value
.
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation("memory_type", "shared"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value, compiled_model

.GetDispatchAnnotation("memory_type"));
ASSERT_TRUE(value
.
...
.GetDispatchAnnotation(0, "nonexistent"));
EXPECT_FALSE(value
.
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation(0, "to_remove", "value"));
...
.GetDispatchAnnotation(0, "to_remove"));
ASSERT_TRUE(value_before
.
...
.RemoveDispatchAnnotation(0, "to_remove"));

// Verify it's gone
LITERT_ASSERT_OK_AND_ASSIGN(
        auto value_after, compiled_model

.GetDispatchAnnotation(0, "to_remove"));
EXPECT_FALSE(value_after
.
...
LITERT_ASSERT_OK(
        compiled_model
.RemoveDispatchAnnotation(0, "never_existed"));
}
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation(0, "accelerator", "gpu"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value1, compiled_model

.GetDispatchAnnotation(0, "accelerator"));
ASSERT_TRUE(value1
.
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation(0, "accelerator", "npu"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value2, compiled_model
...
{
LITERT_ASSERT_OK(compiled_model
.SetDispatchAnnotation(0, "key1", "value1"));
LITERT_ASSERT_OK(compiled_model
.SetDispatchAnnotation(0, "key2", "value2"));
LITERT_ASSERT_OK(compiled_model
.SetDispatchAnnotation(0, "key3", "value3"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto val1, compiled_model

.GetDispatchAnnotation(0, "key1"));
ASSERT_TRUE(val1
.
...
LITERT_ASSERT_OK_AND_ASSIGN(
        auto val2, compiled_model

.GetDispatchAnnotation(0, "key2"));
ASSERT_TRUE(val2
.
...
LITERT_ASSERT_OK_AND_ASSIGN(
        auto val3, compiled_model

.GetDispatchAnnotation(0, "key3"));
ASSERT_TRUE(val3
.
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation(0, "empty_value", ""));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value, compiled_model

.GetDispatchAnnotation(0, "empty_value"));
ASSERT_TRUE(value
.
...
.GetDispatchAnnotation(0, "key-with-dashes"));
ASSERT_TRUE(value
.
...
{
LITERT_ASSERT_OK(compiled_model
.SetDispatchAnnotation("precision", "fp16"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value, compiled_model

.GetDispatchAnnotation("precision"));
ASSERT_TRUE(value
.
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation("to_remove", "value"));

LITERT_ASSERT_OK_AND_ASSIGN(
        auto value_before, compiled_model

.GetDispatchAnnotation("to_remove"));
ASSERT_TRUE(value_before
.
...
LITERT_ASSERT_OK(compiled_model
.RemoveDispatchAnnotation("to_remove"));
...
LITERT_ASSERT_OK(
        compiled_model
.SetDispatchAnnotation("cross_check", "test_value"));

// Since this is signature 0, we should be able to get it by index 0
LITERT_ASSERT_OK_AND_ASSIGN(
        auto value, compiled_model

.GetDispatchAnnotation(0, "cross_check"));
ASSERT_TRUE(value
.
