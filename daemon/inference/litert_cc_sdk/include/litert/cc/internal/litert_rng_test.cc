...
auto _
:
FuzzBlock(kTestIters, std::chrono::milliseconds(50)
)) {
const auto val = gen(device);
ASSERT_LE(val, gen
.
...
High(), NumericLimits<TypeParam>::Max()

);
EXPECT_EQ(data
.

Low(), NumericLimits<TypeParam>::Lowest()
...
LITERT_ASSERT_OK_AND_ASSIGN(const auto buf, data(device,
...
LITERT_ASSERT_OK_AND_ASSIGN(auto buf,

(b.
Call<TypeParam, Functor>(device)
));
EXPECT_EQ(buf
.
