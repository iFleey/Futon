...
namespace litert {
    namespace {
        ...
        .external_tensors_mode, param.precision, param.buffer_storage_type));
...
false, GpuOptions::Precision::kDefault,
GpuOptions::BufferStorageType::kDefault));
...
.
GetOption(kLiteRtEnvOptionTagWebGpuDevice)
);
ABSL_LOG(INFO)
<< "WebGPU device id: "
<< reinterpret_cast
...
.
GetOption(kLiteRtEnvOptionTagWebGpuQueue)
);
ABSL_LOG(INFO)
...
true, GpuOptions::Precision::kFp32,
GpuOptions::BufferStorageType::kTexture2D));
...
device.
GetAdapterInfo(&adapter_info);
ASSERT_OK(webgpu_env
->
Initialize(device, adapter_info
));
...
LITERT_ASSERT_OK_AND_ASSIGN(auto signatures, compiled_model
...
LITERT_ASSERT_OK_AND_ASSIGN(auto input0_bytes, input_tensor_type0
...
EXPECT_TRUE(webpu_tensor_buffer0
.

IsWebGpuMemory()

);
ASSERT_TRUE(webpu_tensor_buffer0
.
...
LITERT_ASSERT_OK_AND_ASSIGN(auto input1_bytes, input_tensor_type1
...
EXPECT_TRUE(webpu_tensor_buffer1
.

IsWebGpuMemory()

);
ASSERT_TRUE(webpu_tensor_buffer1
.
...
input_buffers.
push_back(std::move(webpu_tensor_buffer0)
);
input_buffers.
push_back(std::move(webpu_tensor_buffer1)
);
