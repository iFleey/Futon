...
namespace litert {
    namespace {
        ...
        // Set some values
        LITERT_EXPECT_OK(runtime_options
        .SetInferencePowerState(3));
        LITERT_EXPECT_OK(runtime_options
        .SetInferencePriority(5));

        // Verify the values
        EXPECT_THAT(runtime_options
        .

        GetInferencePowerState(), IsOkAndHolds(3)

        );
        EXPECT_THAT(runtime_options
        .

        GetInferencePriority(), IsOkAndHolds(5)
    ...
    GetInferencePowerState(), IsOkAndHolds(6)

    );
    EXPECT_THAT(options
    .

    GetInferenceMemoryPowerState(), IsOkAndHolds(3)

    );
    EXPECT_THAT(options
    .

    GetInferencePriority(), IsOkAndHolds(-1)

    );
    EXPECT_THAT(options
    .

    GetPreferCoherent(), IsOkAndHolds(false)
