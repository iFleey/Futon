...
namespace {
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(WebNnOptions
    options,
    ...
    EXPECT_EQ(options
    .
    SetDevicePreference(kDevicePreference), kLiteRtStatusOk
    );

    LITERT_ASSERT_OK_AND_ASSIGN(LiteRtWebNnOptionsPayload
    payload,
    options.

    GetData<LiteRtWebNnOptionsPayloadT>()
...
EXPECT_EQ(options
.
SetPowerPreference(kPowerPreference), kLiteRtStatusOk
);
...
EXPECT_EQ(options
.
SetPrecision(kPrecision), kLiteRtStatusOk
);
