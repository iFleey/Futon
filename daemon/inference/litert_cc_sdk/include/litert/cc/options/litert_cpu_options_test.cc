...
// A test option implementation to discriminate against CpuOptions.
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(CpuOptions
    options,
...
LITERT_ASSERT_OK_AND_ASSIGN(NotCpuOptions
original_options,
...
LITERT_EXPECT_OK(options
.SetXNNPackWeightCachePath("a/path"));
EXPECT_THAT(options
.
...
GetXNNPackWeightCacheFileDescriptor(),
        IsOkAndHolds(-1)
...
LITERT_EXPECT_OK(options
.SetXNNPackWeightCacheFileDescriptor(1234));
EXPECT_THAT(options
.
...
GetXNNPackFlags(), IsOkAndHolds(3)
