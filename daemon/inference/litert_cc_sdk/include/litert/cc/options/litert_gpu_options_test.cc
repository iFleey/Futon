...
namespace litert::ml_drift {
    namespace {
        ...
        LITERT_ASSERT_OK_AND_ASSIGN(GpuOptions
        options,

        GpuOptions::Create()

        );
        LITERT_ASSERT_OK_AND_ASSIGN(LiteRtGpuOptionsPayload
        payload,
        options.

        GetData<LiteRtGpuOptionsPayloadT>()
...
LITERT_EXPECT_OK(options
.EnableAllowSrcQuantizedFcConvOps(true));
...
.EnableAllowSrcQuantizedFcConvOps(true));
LITERT_EXPECT_OK(options
.EnableAllowSrcQuantizedFcConvOps(false));
bool allow_src_quantized_fc_conv_ops = true;
...
options.
SetPrecision(GpuOptions::Precision::kFp16);
...
EXPECT_EQ(serialize_program_cache,
true);
...
EXPECT_EQ(serialize_external_tensors,
false);
...
EXPECT_EQ(prefer_texture_weights,
false);
...
EXPECT_EQ(hint_fully_delegated_to_single_delegate,
false);
