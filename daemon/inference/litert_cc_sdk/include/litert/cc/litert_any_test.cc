...
ASSERT_EQ(std::get<bool>(variant_true),
true);
...
ASSERT_EQ(std::get<bool>(variant_false),
false);
...
true)));
ASSERT_EQ(any_true
.type, kLiteRtAnyTypeBool);
ASSERT_EQ(any_true
.bool_value, true);

LITERT_ASSERT_OK_AND_ASSIGN(
        auto any_false, litert::ToLiteRtAny(litert::LiteRtVariant(
...
ASSERT_EQ(std::get<int64_t>(variant),
1234);
...
LiteRtAny litert_any;
litert_any.
type = kLiteRtAnyTypeReal;
litert_any.
real_value = 123.4;
auto variant = litert::ToStdAny(litert_any);
ASSERT_TRUE(std::holds_alternative<double>(variant)
);
ASSERT_EQ(std::get<double>(variant),
123.4);
...
ASSERT_EQ(any_double
.type, kLiteRtAnyTypeReal);
EXPECT_NEAR(any_double
.real_value, 1.2, 1e-7);
}
...
"test")));
ASSERT_EQ(any_char_p
.type, kLiteRtAnyTypeString);
EXPECT_STREQ(any_char_p
.str_value, "test");
}
...
ASSERT_EQ(std::get<void *>(variant),
const_cast
<void *>(kTestPtr)
);
