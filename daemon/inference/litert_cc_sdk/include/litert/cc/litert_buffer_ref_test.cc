...
str(), StartsWith("MutableBufferRef")
...
Span(), ElementsAreArray(MakeConstFbData(kExpData))
...
str(), StartsWith("OwningBufferRef")
...
auto raw = MakeInternalTestBuffer<Mallocator < uint8_t>>
(kData);
...
auto malloc_buffer = MakeInternalTestBuffer<Mallocator < uint8_t>>
(kData);
...
"SOMERawBuffer");
EXPECT_EQ(FbBufToStr(const_buf),
"SomeRawBuffer");
}
...
0);
EXPECT_EQ(absl::string_view(data, size),
"BAZ");
