...
+ (void)testBasicMetalTest:(BOOL)asyncMode externalTensorsMode:(BOOL)externalTensorsMode {
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    options, CreateGpuOptions(externalTensorsMode));
    XCTAssertTrue(options);
    LITERT_ASSERT_OK_AND_ASSIGN(
            auto
    compiled_model, litert::CompiledModel::Create(env, modelFilePath.UTF8String, options));
    XCTAssertEqual(compiled_model.GetNumSignatures(), 1);
    XCTAssertTrue(compiled_model);

    LITERT_ASSERT_OK_AND_ASSIGN(auto
    input_buffers, compiled_model.CreateInputBuffers());

    LITERT_ASSERT_OK_AND_ASSIGN(auto
    output_buffers, compiled_model.CreateOutputBuffers());

    // // Fill model inputs.
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    input_names, compiled_model.GetSignatureInputNames());
    ...
    // Check model output.
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    output_names, compiled_model.GetSignatureOutputNames());
...
+ (void)testMetalPipelineTest:(BOOL)asyncMode1stModel
    ...
    // Create 1st model.
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    compiled_model_1, litert::CompiledModel::Create(
            env, modelFilePath.UTF8String, options));
    XCTAssertEqual(compiled_model_1.GetNumSignatures(), 1);
    XCTAssertTrue(compiled_model_1);
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    input_buffers_1, compiled_model_1.CreateInputBuffers());
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    output_buffers_1, compiled_model_1.CreateOutputBuffers());

    // Create 2nd model.
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    compiled_model_2, litert::CompiledModel::Create(
            env, modelFilePath.UTF8String, options));
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    output_buffers_2, compiled_model_2.CreateOutputBuffers());

    // Fill model inputs for 1st model.
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    input_names, compiled_model_1.GetSignatureInputNames());
    XCTAssertEqual(input_names.size(), 2);
    XCTAssertEqualObjects([NSString stringWithUTF8String:input_names.at(0).data()], @"arg0");
    XCTAssertEqualObjects([NSString stringWithUTF8String:input_names.at(1).data()], @"arg1");
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    output_names, compiled_model_2.GetSignatureOutputNames());
    XCTAssertEqual(output_names.size(), 1);
    XCTAssertEqualObjects([NSString stringWithUTF8String:output_names.at(0).data()], @"tfl.add");
...
- (void)testCompiledModelGpuEnvironment {
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    options, CreateGpuOptions(/*external_tensors_mode=*/false));
    LITERT_ASSERT_OK_AND_ASSIGN(
            auto
    compiled_model, litert::CompiledModel::Create(*env, modelFilePath.UTF8String, options));

    LITERT_ASSERT_OK_AND_ASSIGN(auto
    env_options, env->GetOptions());
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    metal_device_id,
...
- (void)testCompiledModelGpuBasicAdd3dCstInt32 {
    constexpr
    const int32_t kInt32TestInput0Tensor[] = {1, 2, 3, 4, 5, 6};
    constexpr
    const int32_t kInt32TestOutputTensor[] = {11, 22, 33, 44, 55, 66};
    constexpr
    const size_t kInt32TestInput0Size = 6;
    constexpr
    const size_t kInt32TestOutputSize = 6;
...
- (void)testCompiledModelexternalTensorBinding {
    ...
    alignas(LITERT_HOST_MEMORY_BUFFER_ALIGNMENT)
    float kWeightTensor[] = {1.0f, 2.0f};
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(litert::TensorBufferRequirements
    requirements,
            compiled_model.GetInputBufferRequirements(0));
    LITERT_ASSERT_OK_AND_ASSIGN(auto
    buffer_type, requirements.SupportedTypes());
    LITERT_ASSERT_OK_AND_ASSIGN(
            TensorBuffer
    arg0_buffer,
