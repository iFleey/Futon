...
namespace litert {
    namespace {
        ...
        // Environment setup.
        LITERT_ASSERT_OK_AND_ASSIGN(Environment
        env, litert::Environment::Create( {
    }
    ));
    ...
    input_buffer_requirements_arg0,
    compiled_model.GetInputBufferRequirements(/*input_name=*/"arg0"));
    LITERT_ASSERT_OK_AND_ASSIGN(
            std::vector<TensorBufferType>
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(std::vector<TensorBufferType>
    output_buffer_types,
    output_buffer_requirements.
    ...
    LITERT_ASSERT_OK_AND_ASSIGN(std::vector<TensorBuffer>
    input_buffers,
    compiled_model.
    ...
    // Check model output.
{
    ...
    ++i) {
    ABSL_LOG(INFO)
    << "Result: " << output[i] << "\t" << kTestOutputTensor[i];
}
...
.
GetSignatureInputNames(signature_key)
);
absl::flat_hash_map <absl::string_view, TensorBuffer> input_map;
for (
const auto &input_name
: input_names) {
LITERT_ASSERT_OK(compiled_model
.
ResizeInputTensor(
...
input_buffer,
compiled_model.
CreateInputBuffer(signature_key, input_name
));
LITERT_ASSERT_OK_AND_ASSIGN(RankedTensorType
buffer_type,
input_buffer.
...
output_buffer,
compiled_model.
CreateOutputBuffer(signature_key,
"tfl.add"));
LITERT_ASSERT_OK_AND_ASSIGN(RankedTensorType
buffer_type,
output_buffer.
...
LITERT_ASSERT_OK_AND_ASSIGN(
        auto input_names, compiled_model
...
input_buffer_requirements_arg0,
compiled_model.
GetInputBufferRequirements(signature_index,
/*input_name=*/"arg0"));
...
output_buffer_requirements,
compiled_model.
GetOutputBufferRequirements(signature_index,
/*output_name=*/"tfl.add"));
...
input_buffers,
compiled_model.
CreateInputBuffers(signature_index)
);
...
output_buffers,
compiled_model.
CreateOutputBuffers(signature_index)
);
...
// Execute model with input and output buffers.
compiled_model.
Run(signature_index, input_buffers, output_buffers
);
...
// Create and fill input and output buffers.
LITERT_ASSERT_OK_AND_ASSIGN(TensorBuffer
input_buffer0,
...
absl::flat_hash_map<absl::string_view, TensorBuffer> input_map;
input_map["arg0"] =
std::move(input_buffer0);
input_map["arg1"] =
std::move(input_buffer1);

absl::flat_hash_map<absl::string_view, TensorBuffer> output_map;
output_map["tfl.add"] =
std::move(output_buffer0);
...
input_buffers,
compiled_model.
CreateInputBuffers(compiled_model
.
...
output_buffers,
compiled_model.
CreateOutputBuffers(compiled_model
.
...
bool async;
compiled_model.
RunAsync(compiled_model
.

DefaultSignatureKey(), input_buffers,
...
LITERT_ASSIGN_OR_ABORT(Options
compilation_options,
...
compilation_options.
SetHardwareAccelerators(HwAccelerators::kCpu);
...
// Create profiler.
LITERT_ASSERT_OK_AND_ASSIGN(auto profiler, compiled_model
...
LITERT_ASSERT_OK_AND_ASSIGN(auto events, profiler
...
LITERT_ASSERT_OK_AND_ASSIGN(events, profiler
.
...
requirements,
compiled_model.
GetInputBufferRequirements(/*input_index=*/size_t(0)
));
LITERT_ASSERT_OK_AND_ASSIGN(size_t
...
requirements,
compiled_model.
GetInputBufferRequirements(/*signature_index=*/size_t(0),
...
req0,
compiled_model.
GetInputBufferRequirements(size_t(0)
));
LITERT_ASSERT_OK_AND_ASSIGN(size_t
size0, req0.
...
req1,
compiled_model.
GetInputBufferRequirements(size_t(1)
));
LITERT_ASSERT_OK_AND_ASSIGN(size_t
size1, req1.
...
input_buffers.
push_back(std::move(input_buffer0)
);
input_buffers.
push_back(std::move(input_buffer1)
);
...
out_req,
compiled_model.
GetOutputBufferRequirements(size_t(0)
));
LITERT_ASSERT_OK_AND_ASSIGN(size_t
out_size, out_req.
...
output_buffers.
push_back(std::move(output_buffer)
);
...
ASSERT_TRUE(input_buffers[0]
.
Write<float>(absl::MakeConstSpan(test_input)
));
ASSERT_TRUE(input_buffers[1]
.
Write<float>(absl::MakeConstSpan(test_input)
...
LITERT_ASSERT_OK_AND_ASSIGN(Options
compilation_options,
...
LITERT_ASSERT_OK(
        runtime_options
.
SetErrorReporterMode(kLiteRtErrorReporterModeBuffer)
);
...
LITERT_ASSERT_OK(compiled_model
.ReportError("Simple error message"));

// Test 2: ReportError with formatting - integers
LITERT_ASSERT_OK(compiled_model
.ReportError("Error code: %d", 404));

// Test 3: ReportError with formatting - strings
LITERT_ASSERT_OK(
        compiled_model
.ReportError("Failed operation: %s", "tensor_allocation"));
...
LITERT_ASSERT_OK_AND_ASSIGN(std::string
error_messages,
compiled_model.
...
LITERT_ASSERT_OK(compiled_model
.ReportError("This goes to stderr: %d", 123));
...
LITERT_ASSERT_OK(
        runtime_options
.
SetErrorReporterMode(kLiteRtErrorReporterModeNone)
);
...
LITERT_ASSERT_OK(
        compiled_model
.ReportError("This should be ignored: %d", 999));
...
ASSERT_EQ(output_buffers
.

size(),

2);  // normal_output and constant_output
...
ASSERT_TRUE(
        input_buffers[0]
.
Write<float>(absl::MakeConstSpan(input_data, 2)
));
...
int i = 0;
i < 2; i++) {
LITERT_ASSERT_OK_AND_ASSIGN(auto size, output_buffers[i]
...
ASSERT_NE(constant_output_idx,
-1) << "Could not find constant output";
ASSERT_NE(normal_output_idx,
-1) << "Could not find normal output";
...
ABSL_LOG(INFO)
<< "Constant output tensor test passed. Values: ["
...
ASSERT_TRUE(
        input_buffers[0]
.
Write<float>(absl::MakeConstSpan(input_data2, 2)
));
LITERT_ASSERT_OK(
        compiled_model
.
...
compilation_options.
SetHardwareAccelerators(HwAccelerators::kCpu);
LITERT_ASSERT_OK(compilation_options
...
LITERT_ASSERT_OK(
        arg0_buffer
.
Write<float>(absl::MakeConstSpan(kInputTensor, 2)
));
input_map["arg0"] =
std::move(arg0_buffer);
...
LITERT_ASSERT_ERROR(compiled_model
.
ResizeInputTensor(
        /*input_index=*/size_t(0), absl::MakeConstSpan(new_dims)
));
