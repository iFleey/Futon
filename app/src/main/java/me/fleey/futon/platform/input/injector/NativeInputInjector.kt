 ...
 */
@Single
class NativeInputInjector : InputInjector {

  override val method: InputMethod = InputMethod.NATIVE_IOCTL
  override val estimatedLatencyMs: Int = 5
  ...
  suspend fun getDevicePath(): String? = null

  suspend fun getMTProtocol(): MTProtocol = MTProtocol.SINGLE_TOUCH

  suspend fun getMaxTouchPoints(): Int = 1

  suspend fun getScreenBounds(): Pair<Int, Int>? = null

  suspend fun release() {
    // No-op for stub
  }

  fun getLastInitError(): GestureResult.Failure? = createUnavailableResult()

  suspend fun getNativeErrorInfo(): String = "Stub - use DaemonBinderClient for input injection"

  suspend fun forceReinit() {
    // No-op for stub
  }
