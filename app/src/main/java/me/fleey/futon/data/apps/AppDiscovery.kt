...
interface AppDiscovery {
  ...
  suspend fun findAppByName(name: String): InstalledApp?
  suspend fun getAppByPackage(packageName: String): InstalledApp?
  fun observeAppChanges(): Flow<List<InstalledApp>>
  suspend fun refresh()
  suspend fun getAIContext(): String?
  fun isPermissionGranted(): Boolean
  suspend fun launchApp(packageName: String): Boolean
  suspend fun detectAppLaunchIntent(taskDescription: String): InstalledApp?
}
